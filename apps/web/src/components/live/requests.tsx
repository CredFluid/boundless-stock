...
export function RequestHistory({ name }: { name: string }) {
  ...
  return (
    <Card>
      <CardHeader
        title="Trade history"
        subtitle="Every request placed from a mirror, read from each mirror's request contract. Pending requests are re-read until they settle."
            ...
            {shown.length === 0 ? (
              ...
              shown.map((r) => (
                <tr key={r.key}>
                  ...
                  <td className="px-5 py-2 font-mono text-xs whitespace-nowrap text-muted" title={r.user}>{shortAddress(r.user)}</td>
                  <td className="px-5 py-2 whitespace-nowrap">
